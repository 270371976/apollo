...
function run_bazel() {
  ...
  if [[ "$(uname -m)" == "x86_64" ]]; then
    job_args="--copt=-mavx2 --host_copt=-mavx2 --jobs=16 --local_ram_resources=HOST_RAM*0.7"
  else
    job_args="--copt=-march=native --host_copt=-march=native --jobs=16 --local_ram_resources=HOST_RAM*0.7 --copt=-fPIC --host_copt=-fPIC"
  fi
  set -x
