...
size_t RDBHandler::pkgId2size(unsigned int pkgId, bool extended) {
  switch (pkgId) {
...
std::string RDBHandler::pkgId2string(unsigned int pkgId) {
  switch (pkgId) {
  case RDB_PKG_ID_START_OF_FRAME:
...
std::string RDBHandler::coordType2string(unsigned int type) {
  switch (type) {
...
std::string RDBHandler::objectCategory2string(unsigned int id) {
  switch (id) {
...
unsigned int RDBHandler::objectString2category(const std::string& name) {
...
std::string RDBHandler::objectType2string(unsigned int id) {
  switch (id) {
...
unsigned int RDBHandler::objectString2type(const std::string& name) {
...
void RDBHandler::printMessage(RDB_MSG_t* msg, bool details, bool binDump, bool csv, bool csvHeader) {
  if (!msg) {
  ...
  if (!csv && !csvHeader) {
    ...
    fprintf(
      stderr,
      "  message: version = 0x%04x, simTime = %.3f, simFrame = %d, headerSize = %d, dataSize = %d\n",
      msg->hdr.version,
      msg->hdr.simTime,
      msg->hdr.frameNo,
      msg->hdr.headerSize,
...
void RDBHandler::printMessageEntry(RDB_MSG_ENTRY_HDR_t* entryHdr, bool details, bool csv, bool csvHeader) {
  if (!entryHdr)
    return;
  ...
  if (!csv && !csvHeader) {
    fprintf(
      stderr,
      "    entry: pkgId = %2d (%s), headersize = %d, dataSize = %d, elementSize = %d, noElements = %d, flags = 0x%x\n",
      entryHdr->pkgId,
      pkgId2string(entryHdr->pkgId).c_str(),
      entryHdr->headerSize,
      entryHdr->dataSize,
      entryHdr->elementSize,
      noElements,
      entryHdr->flags);
...
bool RDBHandler::printMessageEntryElement(RDB_MSG_ENTRY_HDR_t* entryHdr, void* dataPtr, unsigned char ident, bool csv, bool csvHeader) {
  if (!entryHdr || !dataPtr)
    return false;
...
bool RDBHandler::retrievePrimaryId(unsigned short elementType, void* element, unsigned int& primaryId) {
  if (!element)
    return false;
...
void* RDBHandler::addPackage(RDB_MSG_t*& msg, const double& simTime, const unsigned int& simFrame, unsigned int pkgId, unsigned int noElements, bool extended, size_t trailingData, bool isCustom) {
  if (!noElements)
    return 0;
...
void* RDBHandler::addCustomPackage(RDB_MSG_t*& msg, const double& simTime, const unsigned int& simFrame, unsigned int pkgId, unsigned int noElements, size_t elementSize) {
  return addPackage(msg, simTime, simFrame, pkgId, noElements, false, elementSize, true);
}

void* RDBHandler::getFirstEntry(RDB_MSG_t* msg, unsigned int pkgId, unsigned int& noElements, bool extended) {
...
RDB_MSG_ENTRY_HDR_t* RDBHandler::getEntryHdr(RDB_MSG_t* msg, unsigned int pkgId, bool extended) {
  if (!msg)
    return 0;
...
char* RDBHandler::getIdentString(unsigned char ident) {
...
void RDBHandler::print(const RDB_GEOMETRY_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_COORD_SYSTEM_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_COORD_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
  ...
  if (csv) {
    fprintf(stderr, "%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%#23x,%23d,%23d,", info.x, info.y, info.z, info.h, info.p, info.r, info.flags, info.type, info.system);
    return;
  }
...
void RDBHandler::print(const RDB_ROAD_POS_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(
      stderr,
      "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,",
      "playerId",
      "roadId",
      "laneId",
      "flags",
  ...
  if (csv) {
    fprintf(
      stderr,
      "%23d,%23d,%23d,%#23x,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%23d,%+.16e,",
      info.playerId,
      info.roadId,
      info.laneId,
      info.flags,
  ...
  fprintf(
    stderr,
    "%s%8d %6d %6d  0x%02x %+10.3f %+10.3f   %+10.3f %+10.3f %+10.3f %+10.3f %9d %+10.3f\n",
    identStr,
    info.playerId,
    info.roadId,
    info.laneId,
...
void RDBHandler::print(const RDB_LANE_INFO_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(
      stderr,
      "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,",
      "roadId",
      "id",
      "neighborMask",
      "leftLaneId",
  ...
  if (csv) {
    fprintf(
      stderr,
      "%23d,%23d,%#23x,%23d,%23d,%23d,%23d,%23d,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%23d,",
      info.roadId,
      info.id,
      info.neighborMask,
      info.leftLaneId,
  ...
  fprintf(
    stderr,
    "%s%8d %6d %6d  0x%02x %+10d %+10d %+8d %+8d %+8d %+10.3f %+10.6f %+10.6f %+10.6f %+10.6f %+8d\n",
    identStr,
    info.playerId,
    info.roadId,
    info.id,
...
void RDBHandler::print(const RDB_ROADMARK_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(
      stderr,
      "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,",
      "playerId",
      "id",
      "prevId",
      "nextId",
  ...
  if (csv) {
    fprintf(
      stderr,
      "%23d,%23d,%23d,%23d,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%23d,%23d,",
      info.playerId,
      info.id,
      info.prevId,
      info.nextId,
...
void RDBHandler::print(const RDB_OBJECT_CFG_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "id", "category", "type", "modelId", "name", "modelName", "fileName", "flags");
    return;
  }

  if (csv) {
    fprintf(stderr, "%23d,%23d,%23d,%23d,%23s,%23s,%23s,%#23x,", info.id, info.category, info.type, info.modelId, info.name, info.modelName, info.fileName, info.flags);
    return;
  }
...
void RDBHandler::print(const RDB_OBJECT_STATE_t& state, bool extended, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
  ...
  if (csv) {
    fprintf(stderr, "%+23d,%23d,%23d,%#23x,%23s,", state.base.id, state.base.category, state.base.type, state.base.visMask, state.base.name);
    print(state.base.geo, ident + 4, csv, csvHeader);
    print(state.base.pos, ident + 4, csv, csvHeader);
...
void RDBHandler::print(const RDB_VEHICLE_SYSTEMS_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,", "playerId", "lightMask", "steering", "steeringWheelTorque");
    return;
  }

  if (csv) {
    fprintf(stderr, "%23d,%#23x,%+.16e,%+.16e,", info.playerId, info.lightMask, info.steering, info.steeringWheelTorque);
    return;
  }
...
void RDBHandler::print(const RDB_VEHICLE_SETUP_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,", "playerId", "mass", "wheelBase");
    return;
  }

  if (csv) {
    fprintf(stderr, "%23d,%+.16e,%+.16e,", info.playerId, info.mass, info.wheelBase);
    return;
  }
...
void RDBHandler::print(const RDB_ENGINE_t& info, bool extended, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,", "playerId", "rps", "load");

    if (!extended)
      return;

    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "rpsStart", "torque", "torqueInner", "torqueMax", "torqueFriction", "fuelCurrent", "fuelAverage");

    return;
  }

  if (csv) {
    fprintf(stderr, "%23d,%+.16e,%+.16e,", info.base.playerId, info.base.rps, info.base.load);
    ...
    fprintf(
      ...
      info.ext.rpsStart,
      info.ext.torque,
      info.ext.torqueInner,
      info.ext.torqueMax,
      info.ext.torqueFriction,
      info.ext.fuelCurrent,
      info.ext.fuelAverage);
...
void RDBHandler::print(const RDB_DRIVETRAIN_t& info, bool extended, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,", "playerId", "gearBoxType", "driveTrainType", "gear");

    if (!extended)
      return;

    fprintf(stderr, "%23s,%23s,%23s,", "torqueGearBoxIn", "torqueCenterDiffOut", "torqueShaft");

    return;
  }

  if (csv) {
    fprintf(stderr, "%23d,%23d,%23d,%23d,", info.base.playerId, info.base.gearBoxType, info.base.driveTrainType, info.base.gear);

    if (!extended)
      return;

    fprintf(stderr, "%+.16e,%+.16e,%+.16e,", info.ext.torqueGearBoxIn, info.ext.torqueCenterDiffOut, info.ext.torqueShaft);
...
void RDBHandler::print(const RDB_WHEEL_t& info, bool extended, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "playerId", "id", "flags", "radiusStatic", "springCompression", "rotAngle", "slip", "steeringAngle");
    ...
    fprintf(
      stderr,
      "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,",
      "vAngular",
      "forceZ",
      "forceLat",
      "forceLong",
  ...
  if (csv) {
    fprintf(
      stderr,
      "%23d,%23d,%#23x,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,",
      info.base.playerId,
      info.base.id,
      info.base.flags,
      info.base.radiusStatic,
      info.base.springCompression,
      info.base.rotAngle,
      info.base.slip,
      info.base.steeringAngle);
    ...
    fprintf(
      ...
      info.ext.forceLat,
      info.ext.forceLong,
      info.ext.forceTireWheelXYZ[0],
      info.ext.forceTireWheelXYZ[1],
      info.ext.forceTireWheelXYZ[2],
      info.ext.radiusDynamic,
      info.ext.brakePressure,
      info.ext.torqueDriveShaft,
      info.ext.damperSpeed,
      info.ext.vAngular,
      info.ext.forceZ,
      info.ext.forceLat,
      info.ext.forceLong);
...
void RDBHandler::print(const RDB_PED_ANIMATION_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_SENSOR_STATE_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "id", "type", "hostCategory", "name", "fovH", "fovV", "clipNear", "clipFar");
    print(info.pos, ident + 4, csv, csvHeader);
  ...
  if (csv) {
    fprintf(stderr, "%23d,%23d,%23d,%23s,%+.16e,%+.16e,%+.16e,%+.16e,", info.id, info.type, info.hostCategory, info.name, info.fovHV[0], info.fovHV[1], info.clipNF[0], info.clipNF[1]);
    print(info.pos, ident + 4, csv, csvHeader);
    print(info.originCoordSys, ident + 4, csv, csvHeader);
    return;
  }
...
void RDBHandler::print(const RDB_SENSOR_OBJECT_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "category", "type", "flags", "id", "sensorId", "dist", "occlusion", "deltaLane");
    print(info.sensorPos, ident + 4, csv, csvHeader);
    return;
  }

  if (csv) {
    fprintf(stderr, "%23d,%23d,%#23x,%23d,%23d,%+.16e,%23d,%23d,", info.category, info.type, info.flags, info.id, info.sensorId, info.dist, info.occlusion, info.deltaLanePos);
    print(info.sensorPos, ident + 4, csv, csvHeader);
    return;
  }
...
void RDBHandler::print(const RDB_CAMERA_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "id", "width", "height", "clipNear", "clipFar", "focalX", "focalY", "principalX", "principalY");
    print(info.pos, ident + 4, csv, csvHeader);
    return;
  }
  ...
  if (csv) {
    fprintf(
      stderr, "%23d,%23d,%23d,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,", info.id, info.width, info.height, info.clipNear, info.clipFar, info.focalX, info.focalY, info.principalX, info.principalY);
    print(info.pos, ident + 4, csv, csvHeader);
    return;
  }
...
void RDBHandler::print(const RDB_CONTACT_POINT_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_TRAFFIC_SIGN_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,", "id", "playerId", "roadDist");
    print(info.pos, ident + 4, csv, csvHeader);
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "type", "subType", "value", "state", "readability", "occlusion", "addOnId", "minLane", "maxLane");
    return;
  }
  ...
  if (csv) {
    ...
    print(info.pos, ident + 4, csv, csvHeader);
    fprintf(stderr, "%23d,%23d,%+.16e,%23d,%23d,%23d,%23d,%23d,%23d,", info.type, info.subType, info.value, info.state, info.readability, info.occlusion, info.addOnId, info.minLane, info.maxLane);
    return;
  }
...
void RDBHandler::print(const RDB_ROAD_STATE_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_IMAGE_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "id", "width", "height", "pixelSize", "pixelFormat", "cameraId", "imgSize", "colorR", "colorG", "colorB", "colorA");
    return;
  }
  ...
  if (csv) {
    fprintf(
      stderr,
      "%23d,%23d,%23d,%23d,%23d,%23d,%23d,%23d,%23d,%23d,%23d,",
      info.id,
      info.width,
      info.height,
      info.pixelSize,
...
void RDBHandler::print(const RDB_LIGHT_SOURCE_t& info, bool extended, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,", "id", "templateId", "state", "playerId", "flags");
    ...
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "near", "far", "left", "right", "bottom", "top", "int1", "int2", "int3", "atten0", "atten1", "atten2");

    return;
  }

  if (csv) {
    fprintf(stderr, "%23d,%23d,%23d,%23d,%23d,", info.base.id, info.base.templateId, info.base.state, info.base.playerId, info.base.flags);
    ...
    fprintf(
      ...
      info.ext.nearFar[0],
      info.ext.nearFar[1],
      info.ext.frustumLRBT[0],
      info.ext.frustumLRBT[1],
      info.ext.frustumLRBT[2],
      info.ext.frustumLRBT[3],
      info.ext.intensity[0],
      info.ext.intensity[1],
      info.ext.intensity[2],
      info.ext.atten[0],
      info.ext.atten[1],
      info.ext.atten[2]);
...
void RDBHandler::print(const RDB_ENVIRONMENT_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,", "visibility", "timeOfDay", "brightness", "precipitation", "cloudState", "flags");
    return;
  }

  if (csv) {
    fprintf(stderr, "%+.16e,%23d,%+.16e,%23d,%23d,%#23x,", info.visibility, info.timeOfDay, info.brightness, info.precipitation, info.cloudState, info.flags);
    return;
  }
...
void RDBHandler::print(const RDB_TRIGGER_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_DRIVER_CTRL_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(
      stderr,
      "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s",
      "playerId",
      "steeringWheelV",
      "steeringWheel",
      "steeringSpeedV",
  ...
  if (csv) {
    fprintf(
      stderr,
      "%23d,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%+.16e,%23d,%23d,%23d,%#23x,%23d,%#23x,",
      info.playerId,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_STEERING_WHEEL) ? 1 : 0,
      info.steeringWheel,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_STEERING_SPEED) ? 1 : 0,
      info.steeringSpeed,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_THROTTLE) ? 1 : 0,
      info.throttlePedal,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_BRAKE) ? 1 : 0,
      info.brakePedal,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_CLUTCH) ? 1 : 0,
      info.clutchPedal,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_TGT_ACCEL) ? 1 : 0,
      info.accelTgt,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_TGT_STEERING) ? 1 : 0,
      info.steeringTgt,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_CURVATURE) ? 1 : 0,
      info.curvatureTgt,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_STEERING_TORQUE) ? 1 : 0,
      info.steeringTorque,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_ENGINE_TORQUE) ? 1 : 0,
      info.engineTorqueTgt,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_TGT_SPEED) ? 1 : 0,
      info.speedTgt,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_GEAR) ? 1 : 0,
      info.gear,
      (info.validityFlags & RDB_DRIVER_INPUT_VALIDITY_FLAGS) ? 1 : 0,
      info.flags,
      info.sourceId,
      info.validityFlags);
    return;
...
void RDBHandler::print(const RDB_TRAFFIC_LIGHT_t& info, bool extended, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_SYNC_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_DRIVER_PERCEPTION_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_FUNCTION_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_ROAD_QUERY_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_POINT_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_TRAJECTORY_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_DYN_2_STEER_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,", "playerId", "state", "cmd", "effects", "torque", "friction", "damping", "stiffness", "velocity", "angle");
    return;
  }
  ...
  if (csv) {
    fprintf(
      stderr,
      "%23d,%#23x,%#23x,%#23x,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,",
      info.playerId,
      info.state,
      info.cmd,
      info.effects,
...
void RDBHandler::print(const RDB_STEER_2_DYN_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
  ...
  if (csv) {
    fprintf(stderr, "%23d,%#23x,%+.16e,%+.16e,%+.16e,", info.playerId, info.state, info.angle, info.rev, info.torque);
    return;
  }
...
void RDBHandler::print(const RDB_PROXY_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_MOTION_SYSTEM_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_CUSTOM_SCORING_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
  ...
  if (csv) {
    fprintf(stderr, "%23d,%+.16e,%+.16e,%+.16e,%+.16e,%#23x,%+.16e,", info.playerId, info.pathS, info.roadS, info.fuelCurrent, info.fuelAverage, info.stateFlags, info.slip);
    return;
  }
...
void RDBHandler::print(const RDB_CUSTOM_OBJECT_CTRL_TRACK_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
  ...
  if (csv) {
    fprintf(stderr, "%23d,%#23x,%23d,%23d,%23d,%+.16e,%+.16e,", info.playerId, info.flags, info.posType, info.dir, info.roadId, info.initialRoadDeltaS, info.targetRoadT);
    return;
  }
...
void RDBHandler::print(const RDB_SCP_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_FREESPACE_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(
      stderr,
      "%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,%23s,",
      "playerId",
      "distance",
      "angleLeft",
      "angleRight",
  ...
  if (csv) {
    fprintf(
      stderr,
      "%23d,%+.16e,%+.16e,%+.16e,%+.16e,%+.16e,%23d,%23d,%23d,%23d,",
      info.playerId,
      info.distance,
      info.angleLeft,
      info.angleRight,
...
void RDBHandler::print(const RDB_DYN_EL_SWITCH_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
  ...
  if (csv) {
    fprintf(stderr, "%23d,%23d,%23d,%#23x,", info.objectId, info.elementId, info.scope, info.state);
    return;
  }
...
void RDBHandler::print(const RDB_DYN_EL_DOF_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
  ...
  if (csv) {
    fprintf(stderr, "%23d,%23d,%23d,%23d,%23d,", info.objectId, info.elementId, info.scope, info.validity, info.nValues);
    return;
  }
...
void RDBHandler::print(const RDB_IG_FRAME_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,%23s,", "deltaT", "frameNo");
    return;
  }
...
void RDBHandler::print(const RDB_RT_PERFORMANCE_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::printMatrix(int* pData, unsigned int width, unsigned int height, bool csv, bool csvHeader) {
  if (csvHeader)
    return;
...
void RDBHandler::print(const RDB_RAY_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
...
void RDBHandler::print(const RDB_SYMBOL_STATE_t& info, unsigned char ident, bool csv, bool csvHeader) {
  if (csvHeader) {
    fprintf(stderr, "%23s,", "id");
...
RDBHandler::RDBHandler() : mMsg(0), mShmHdr(0) {
...
void* RDBHandler::addPackage(const double& simTime, const unsigned int& simFrame, unsigned int pkgId, unsigned int noElements, bool extended, size_t trailingData, bool isCustom) {
...
void* RDBHandler::addCustomPackage(const double& simTime, const unsigned int& simFrame, unsigned int pkgId, unsigned int noElements, size_t elementSize) {
  // extend the internal message if no other is given
  return addCustomPackage(mMsg, simTime, simFrame, pkgId, noElements, elementSize);
}

RDB_MSG_t* RDBHandler::getMsg() {
  return mMsg;
}

RDB_MSG_HDR_t* RDBHandler::getMsgHdr() {
  if (!mMsg)
    return 0;
...
size_t RDBHandler::getMsgTotalSize() {
  if (!mMsg)
    return 0;
...
void* RDBHandler::getFirstEntry(unsigned int pkgId, unsigned int& noElements, bool extended) {
  return getFirstEntry(mMsg, pkgId, noElements, extended);
}

RDB_MSG_ENTRY_HDR_t* RDBHandler::getEntryHdr(unsigned int pkgId, bool extended) {
  return getEntryHdr(mMsg, pkgId, extended);
}

bool RDBHandler::shmConfigure(void* startPtr, unsigned int noBuffers, unsigned int totalSize) {
...
bool RDBHandler::shmSetAddress(void* shmAddr) {
  mShmHdr = (RDB_SHM_HDR_t*)shmAddr;

  return mShmHdr != 0;
}

RDB_SHM_HDR_t* RDBHandler::shmGetHdr() {
  return mShmHdr;
}

RDB_SHM_BUFFER_INFO_t* RDBHandler::shmBufferGetInfo(unsigned int index) {
  if (!mShmHdr)
    return 0;
...
void* RDBHandler::shmBufferGetPtr(unsigned int index) {
...
void RDBHandler::shmBufferSetSize(unsigned int index, size_t size) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
size_t RDBHandler::shmBufferGetSize(unsigned int index) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
void RDBHandler::shmBufferSetId(unsigned int index, unsigned int id) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
void RDBHandler::shmBufferSetFlags(unsigned int index, unsigned int flags) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
void RDBHandler::shmBufferAddFlags(unsigned int index, unsigned int flags) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
void RDBHandler::shmBufferReleaseFlags(unsigned int index, unsigned int flags) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
unsigned int RDBHandler::shmBufferGetFlags(unsigned int index) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
bool RDBHandler::shmBufferHasFlags(unsigned int index, unsigned int mask) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
bool RDBHandler::mapMsgToShm(unsigned int index, bool relocateBuffers) {
...
bool RDBHandler::addMsgToShm(unsigned int index, RDB_MSG_t* msg) {
...
unsigned int RDBHandler::shmBufferGetUsedSize(unsigned int index) {
  char* tgt = (char*)shmBufferGetPtr(index);
...
bool RDBHandler::shmBufferClear(unsigned int index, bool force) {
...
bool RDBHandler::shmBufferIsLocked(unsigned int index) {
  // check if buffer is locked
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
bool RDBHandler::shmBufferLock(unsigned int index) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
bool RDBHandler::shmBufferRelease(unsigned int index) {
  RDB_SHM_BUFFER_INFO_t* info = shmBufferGetInfo(index);
...
unsigned int RDBHandler::shmGetNoBuffers() {
  if (!mShmHdr)
    return 0;
...
void RDBHandler::printPackageSizes() {
...
void RDBHandler::parseMessage(RDB_MSG_t* msg) {
  if (!msg)
    return;
...
void RDBHandler::parseMessageEntry(RDB_MSG_ENTRY_HDR_t* entryHdr, const double& simTime, const unsigned int& simFrame) {
  if (!entryHdr)
    return;
...
void RDBHandler::parseMessageEntryInfo(
  const double& simTime,
  const unsigned int& simFrame,
  const unsigned short& pkgId,
...
void RDBHandler::parseStartOfFrame(const double& simTime, const unsigned int& simFrame) {
  fprintf(stderr, "RDBHandler::parseStartOfFrame: simTime = %.3f, simFrame = %d\n", simTime, simFrame);
}

void RDBHandler::parseEndOfFrame(const double& simTime, const unsigned int& simFrame) {
  fprintf(stderr, "RDBHandler::parseEndOfFrame: simTime = %.3f, simFrame = %d\n", simTime, simFrame);
}

void RDBHandler::parseEntry(
  RDB_GEOMETRY_t* data,
  const double& simTime,
  const unsigned int& simFrame,
  const unsigned short& pkgId,
...
void RDBHandler::parseEntry(
  RDB_CUSTOM_OBJECT_CTRL_TRACK_t* data,
  const double& simTime,
  const unsigned int& simFrame,
  const unsigned short& pkgId,
  const unsigned short& flags,
